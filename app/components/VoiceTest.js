...
export default function VoiceTest() {
  ...
  const stopRecording = () => {
    if (
      ...
      window.currentRecorder.state === "recording"
    ) {
  ...
  const testTextToSpeech = async () => {
    ...
    try {
      // Use new TTS handler that handles the updated API format
      const result = await generateAndPlaySpeech(
        testText,
        process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3000",
      );

      // Setup cleanup when audio ends - component manages lifecycle
      result.audio.onended = () => {
        setIsPlaying(false);
        result.cleanup();
      };

      // Also handle pause/abort to stop playing state
