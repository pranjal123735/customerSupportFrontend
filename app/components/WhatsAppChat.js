...
import generateAndPlaySpeech from "../utils/ttsHandler";
...
export default function WhatsAppChat() {
  ...
  const addMessage = (text, sender, type = "text", meta = {}) => {
    const newMessage = {
      id: Date.now(),
      text,
      sender,
  ...
  const processVoiceMessage = async (blob) => {
    ...
    try {
      ...
      const ttsResult = await generateAndPlaySpeech(
        aiResponse,
        process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3000",
      );

      // Setup cleanup when audio ends - store both URL and cleanup function
      ttsResult.audio.onended = () => {
        console.log("✅ AI voice message finished playing");
        // Keep cleanup function available but don't auto-call it
        // User may want to replay, so don't revoke URL yet
      };

      // Add agent voice response with audio URL and cleanup function
      addMessage(`🔊 ${aiResponse}`, "agent", "voice", {
        ...
        <div className="flex items-center space-x-2">
          <div className="flex-1 flex items-center space-x-2">
            <input
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-full text-gray-500 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-whatsapp-green focus:border-transparent"
